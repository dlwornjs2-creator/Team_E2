...
명세: docs/3d_map_spec.md 구현 순서 7번. mesh 파일 자체는 여기서 안 읽는다
...
(front_ui는 TF를 다루지 않는다, CLAUDE.md). 여기선 로컬 점군을 그 값으로
회전·이동시켜 하나로 합치기만 한다.
"""
...
def build_robot_points(links: list[dict] | None) -> np.ndarray:
    ...
    따로 그리면 도형이 6배로 늘어 그만큼 렉의 원인이 된다(PROGRESS.md 2.4절).

    `links`가 없거나 비어 있으면 빈 배열을 돌려준다 — 명세 11장: "robot이
    없거나 비어 있으면 팔을 그리지 않는다. 오류가 아니다."
    """
    cache = _load_all()

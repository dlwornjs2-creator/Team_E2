...
def _zone_box_params(zone: dict):
    """open_ratio를 반영한 현재 중심/회전을 계산한다 (명세 7장).
...
class MapView:
    ...
    def _redraw(self):
        ...
        """
        # 명세 8장 순서(뒤->앞): 그리드 -> 고정 배경 상자 -> zones -> 로봇 팔
        ...
        # 로봇 팔. robot.links 데이터가 없으면 명세 11장대로 그냥 아무것도
    ...
    def update(
        ...
        """폴링마다 호출된다 (open_ratio가 계속 바뀌니 캐시하지 않는다 — 명세 7장).
    ...
    def _on_right_pan_update(self, e: ft.PointerEvent):
        ...
        (`rotated = R@P + (I-R)@pivot`, 명세: projection.py) 역으로 풀어서,

...
┌──────────┬──────────┐
│ 작업 기록 │ 물건 위치 │        ← 탭 (직접 만든 pill 버튼. ft.Tabs 안 씀 —
├──────────┴──────────┴──────┐    와이어프레임 형태에 맞추고 API 변경 위험을
│                            │    피하기 위함, 명세 9장)
│  선택한 탭 내용               │
│                            │
└────────────────────────────┘

명세 9장 기준으로 두 탭 다 db 기반이다:
  작업 기록 = recent_tasks (db tasks 테이블, back_ui가 폴링해서 보내줌)
  물건 위치 = objects (db items 테이블, back_ui가 폴링해서 보내줌)

명세에 있는 "탐색 구역 수"/"파지 결과"(작업 기록), "위치 라벨"(물건 위치),
이벤트 타임라인, 작업별 이미지는 아직 뺐다 — 지금 스키마·db에 그 데이터
자체가 없다(back_ui/README.md "back_ui가 줘야 하는 것" 참고). 나중에
db/back_ui 쪽에 그 데이터가 생기면 여기 표에 칸만 추가하면 된다.
"""

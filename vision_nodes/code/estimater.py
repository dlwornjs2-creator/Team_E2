...
class Any6D:
    ...
    def reset_object(self, mesh=None, symmetry_tfs=None):
        if mesh is None:
            raise ValueError("mesh가 None입니다.")
        ...
        # 1순위: OBJ + MTL + PNG 텍스처를 UV로 정점 색상으로 변환
        visual = getattr(mesh, "visual", None)
        material = getattr(visual, "material", None)
        material_image = getattr(material, "image", None)
        uv = getattr(visual, "uv", None)

        if material_image is not None and uv is not None and len(uv) == len(mesh.vertices):
            if isinstance(material_image, np.ndarray):
                img = np.asarray(material_image)
            else:
                img = np.asarray(material_image.convert("RGB"))

            if img.ndim == 2:
                img = np.repeat(img[..., None], 3, axis=2)
            img = img[..., :3].astype(np.uint8, copy=False)

            uv = np.asarray(uv, dtype=np.float64).copy()
            uv[:, 1] = 1.0 - uv[:, 1]
            uv_pixels_y = np.clip(
                (uv[:, 1] * (img.shape[0] - 1)).astype(np.int64),
                0,
                img.shape[0] - 1,
            )
            uv_pixels_x = np.clip(
                (uv[:, 0] * (img.shape[1] - 1)).astype(np.int64),
                0,
                img.shape[1] - 1,
            )
            colors_rgb = img[uv_pixels_y, uv_pixels_x]
            mesh_o3d.vertex_colors = o3d.utility.Vector3dVector(
                colors_rgb.astype(np.float64) / 255.0
            )
            print(f"[Mesh] PNG texture를 사용합니다: {img.shape[1]}x{img.shape[0]}")

        # 2순위: Any6D InstantMesh OBJ의 vertex color를 그대로 사용
        else:
            raw_vertex_colors = getattr(visual, "vertex_colors", None)
            if raw_vertex_colors is not None and len(raw_vertex_colors) == len(mesh.vertices):
                colors = np.asarray(raw_vertex_colors)
                if colors.ndim == 2 and colors.shape[1] >= 3:
                    colors_rgb = colors[:, :3]
                    if np.issubdtype(colors_rgb.dtype, np.floating) and colors_rgb.max(initial=0) <= 1.0:
                        colors_float = colors_rgb.astype(np.float64)
                    else:
                        colors_float = colors_rgb.astype(np.float64) / 255.0
                    colors_float = np.clip(colors_float, 0.0, 1.0)
                    mesh_o3d.vertex_colors = o3d.utility.Vector3dVector(colors_float)
                    print(f"[Mesh] vertex color를 사용합니다: {len(colors_float)}개 정점")
                else:
                    print("[Mesh 경고] vertex color 배열 형식이 올바르지 않습니다.")
            else:
                print("[Mesh 경고] PNG texture와 vertex color가 모두 없습니다.")

...
class DBClient:
    ...
    def lookup(self, task: RobotTask) -> DBLookupResult:
        """Search by name first, then fall back to class_label."""
        queries: list[dict[str, str]] = []
        if task.name:
            queries.append({"name": task.name})
        if task.class_label:
            queries.append({"class_label": task.class_label})
        ...
        for query in queries:
            ...
            if str(latest_item.get("location", "")).strip():
                return DBLookupResult(True, query, latest_item)

        return DBLookupResult(False, last_query, None)

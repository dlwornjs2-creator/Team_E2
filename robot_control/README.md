...
- `JHOME_POS`까지 충돌 없는지 확인
- Tool `Tool Weight_1`과 TCP `GripperDA_v1`이 제어기에 등록됐는지 확인
- Any6D 위치 단위가 mm인지 확인
- `frame_id=base`와 실제 Base 좌표 방향 확인
- 목표점 및 목표점 위 50 mm 구간의 장애물 확인
- `MIN_DEPTH=2 mm`는 범용 안전 높이가 아니므로 실제 테이블 높이에 맞게 조정
- 첫 시험은 드라이런과 낮은 속도로 수행
- 비상정지 버튼에 즉시 접근 가능한 상태에서 실행

현재 구현은 이동 중 통신 오류가 발생했을 때 자동 retract/home 복구를 보장하지
않습니다. 오류 발생 시 로봇이 접근점이나 grasp 위치에 정지할 수 있으므로 실제
운영 전 별도의 복구 절차를 마련해야 합니다.
